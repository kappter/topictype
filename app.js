...
const TopicTyper = () => {
    const [mode, setMode] = useState('dashboard'); // dashboard, typing, quiz
    ...
    const TypingMode = () => {
        ...
        const [difficulty, setDifficulty] = useState('easy'); // easy, medium, hard, coding
        ...
        useEffect(() => {
            ...
            const getRandomTerm = () => {
                const terms = currentSet.sort((a, b) => (b.preferred ? 1 : 0) - (a.preferred ? 1 : 0) || Math.random() - 0.5);
            ...
            const drawWord = (word) => {
                ...
                const underscores = difficulty === 'easy' ? word.term[0] + '_ '.repeat(word.term.length - 1) : '_ '.repeat(word.term.length);
                ...
                ctx.fillStyle = 'black';
                ctx.fillText(underscores, word.x, word.y);
                ctx.fillStyle = 'blue';
                ctx.fillText(word.definition, word.x, word.y - 20);
                let matchedLength = currentInput.toLowerCase().startsWith(word.term.toLowerCase().slice(0, currentInput.length)) ? currentInput.length : 0;
            ...
            const gameLoop = (time) => {
                ...
                setWaveTime(t => Math.max(0, t - 1 / 60));
                if (waveTime <= 0) {
                    setWave(w => w + 1);
                    setWaveTime(30);
                    setWords([]);
                ...
                })).filter(word => word.y <= canvasRef.current.height));
                words.forEach(drawWord);
                if (words.some(w => w.y > canvasRef.current.height)) {
                    setMisses(m => m + 1);
                    setWords(w => w.filter(word => word.y <= canvasRef.current.height));
                    setCurrentInput('');
                    if (misses >= 5) {
                        setGameOver(true);
                        updateProgress();
            ...
            const handleKeyDown = (e) => {
                ...
                if (e.key === 'Backspace') {
                    ...
                    const activeWord = words.find(w => (difficulty === 'hard' ? w.term : w.term.toLowerCase()).startsWith(difficulty === 'hard' ? currentInput : currentInput.toLowerCase()));
                    if (activeWord) {
                        setCorrectKeystrokes(c => c + 1);
                        if ((difficulty === 'hard' ? currentInput : currentInput.toLowerCase()) === (difficulty === 'hard' ? activeWord.term : activeWord.term.toLowerCase())) {
                            if (difficulty === 'coding' && activeWord.codingConvention !== 'none') {
                                ...
                                if (!regex.test(currentInput)) return;
                            }
        ...
        }, [gameOver, waveTime, misses, currentInput, difficulty]);
        ...
        const updateProgress = (term, correct, time) => {
            setProgress(p => {
                ...
                if (correct) {
                    ...
                    newProgress[term].typing.wpm = (wordsTyped / (totalTime / 60)).toFixed(2);
                }
        ...
        const downloadCertificate = () => {
            ...
            doc.text(`${(totalTime > 0 ? wordsTyped / (totalTime / 60) : 0).toFixed(2)} WPM`, 130, 130);
            ...
            doc.save('TopicTyper_Certificate.pdf');
        };
        ...
        return (
            <div className="container mx-auto p-4">
                <div className="flex justify-between mb-4">
                    <div>
                        <span>Score: {score}</span> | <span>Wave: {wave}</span> | <span>Time: {Math.floor(waveTime)}s</span> | <span>Misses: {misses}</span> | <span>WPM: {(totalTime > 0 ? wordsTyped / (totalTime / 60) : 0).toFixed(2)}</span>
                    </div>
                ...
                {gameOver && (
                    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg text-center">
                            ...
                            <p>WPM: {(totalTime > 0 ? wordsTyped / (totalTime / 60) : 0).toFixed(2)}</p>
    ...
    const QuizMode = () => {
        ...
        useEffect(() => {
            ...
            const generateQuestions = () => {
                const validData = currentSet.filter(item => item.term?.trim() && item.definition?.trim());
